  ...
  var createChild = function(modules, providersFromParent) {
    if (providersFromParent && providersFromParent.length) {
      ...
      providersFromParent.forEach(function(name) {
        ...
        if (providers[name][2] === 'private') {
          var privateChildInjector = providers[name][3].createChild([], [name]);
          var getFromPrivateInjector = annotate(function(key) {
            return privateChildInjector.get(key)
          });
          fromParentModule[name] = [getFromPrivateInjector, name, 'private', privateChildInjector];
        } else {
